    ...
    if form.is_submitted():
        if form.word.data.lower().strip() == cross.dictionary_words[str(id_of_question)][1].lower().strip():
            ...
            return render_template('enter_word.html', quest=cross.dictionary_words[str(id_of_question)][0],
                                   message='Неправильно введенное слово', form=form)
    return render_template('enter_word.html', id=id_of_question, quest=cross.dictionary_words[str(id_of_question)][0],
                           form=form)
